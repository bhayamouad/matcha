<template>
  <div class="the-box">
    ...
    <div id="login-form">
      ...
      <b-button @click="change" type="is-primary" expanded>Change Password</b-button>
...
const validateNpassword = (npassword) => {
    if (!npassword) return { valid: false, error: "The New Password is required" }
    if (!npassword.match(/^(?=.*[a-z])(?=.*[A-Z])(?=.*[\d])(?=.*[^A-Za-z0-9\s]).{8,}/))
      return {
        valid: false,
        error: "Enter a combination of at least 8 numbers, uppercase, lowercase and special charracters (such as @,&...).",
      }
...
export default {
    ...
    methods:{
      async change(){
        ...
        const validCpassword = validateCpassword(this.passwords.cpassword, this.passwords.npassword)
        this.errors.cpassword = validCpassword.error
        if (this.valid) this.valid = validCpassword.valid

        if(this.valid) { 
          const res = await this.$axios.$post('/account/changePassword', this.passwords)
          if(res.message)
